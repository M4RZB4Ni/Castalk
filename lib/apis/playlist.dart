...
import 'package:castalk/models/playlist_model.dart';
...
class PlaylistApi {
  playListCreate(
    ...
    var request = http.Request(
        'POST',
        Uri.parse(BaseApi.baseAddressSlash +
  ...
  Future<EpisodesModel> playListSingle(
    ...
    if (response.statusCode == 200) {
      ...
      return EpisodesModel.fromJson(resp);
    } else {
      debugPrint(response.reasonPhrase);

      return const EpisodesModel();
    }
  }
  ...
  Future<PlayListModel> playListIndex({required var token}) async {
    var headers = {
      'Authorization': 'Bearer $token',
    };

    var request = http.Request('GET',
        Uri.parse(BaseApi.baseAddressSlash + 'api/rest/Castalk/PlayListIndex'));
  ...
  playListDelete({required var token, required var id}) async {
    var headers = {
      'Authorization': 'Bearer $token',
    };
    ...
    var request = http.MultipartRequest(
        'POST',
        Uri.parse(
            BaseApi.baseAddressSlash + 'api/rest/Castalk/PlayListDelete'));
